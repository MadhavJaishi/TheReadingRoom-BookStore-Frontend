  ...
  const submit = async () => {
    try {
        if (Object.values(Data).some(value => !value)) {
            ...
            const response = await axios.post("https://thereadingroom-bookstore.onrender.com/api/v1/add-book", Data, {headers});
            setData({
                url: "",
                title: "",
                author: "",
  ...
  return (
    <div className='h-[100%] p-0 md:p-4'>
        ...
        <div className='p-4 bg-zinc-800 rounded'>
            ...
            <button onClick={submit} className='mt-4 px-3 bg-blue-500 text-white font-semibold py-2 rounded hover:bg-blue-600 transition-all duration-300'>Add Book</button>
        </div>
    </div>
  )
}
